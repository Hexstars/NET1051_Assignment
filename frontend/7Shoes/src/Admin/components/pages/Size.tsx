...
const Size = () => {
  ...
  const [currentData, setCurrentData] = useState<SizeForViews>(
    {} as SizeForViews
  );
  ...
  const { isOpen, isOpenAdd, onCloseAdd, onOpenAdd, onClose, onOpen } =
    useDisclosure();
    ...
    try {
      const isActive =
        filterStatus === "true"
          ...
          : null;
      const pageSize = 6;
      const { sizes, totalCount } = await sizeService.getList(
        page,
        pageSize,
        isActive
      );
  ...
  return (
    <div className="container">
      <div className="d-flex justify-content-start mb-3">
        ...
        <select
          className="form-select mx-3 w-auto"
            ...
            sizes.map((size) => (
              <tr key={size.id}>
                ...
                <td className="align-middle text-center">
                  {size.createdDate
                    ? new Date(size.createdDate).toLocaleDateString("vi-VN")
                    : "Chưa cập nhật"}
                </td>
                <td
                  className={`align-middle text-center ${
                    size.isActive ? "text-success" : "text-danger"
                  }`}
                >
                ...
                <td className="align-middle text-center">
                  ...
                  <button
                    ...
                    onClick={() => onDeleteHandle(size.id)}
                  >
      ...
      <SizeForm
        fetchSize={() => loadData(currentPage)}
        isOpen={isOpen}
        onClose={onClose}
